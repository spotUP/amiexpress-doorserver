 ...
 */
const COMPAT_RE =
  /\b(?:now\s+work(?:s|ing)|works?\s+(?:only\s+)?(?:with|on)|requires?|needs?)\b.{0,12}\b(?:\/?X|amiexpress|fame|daydream|tempest|cnet|\d)/i;
...
const BBS_CONTEXT_RE = /(?:\/X|amiexpress|ami|s!x|fame|daydream|tempest|cnet|for)\s*$/i;
